...
| Device and aggregate client APIs | Implemented | stable client-instance upsert, owned-device enforcement, heartbeat/status, completed-day idempotent aggregate ingest, monotonic current-UTC-day snapshot ingest, and 7/30-day aggregate analytics are wired; daily aggregates may carry a privacy-preserving 24-slot `metadata_json.hourly` histogram (device-local hour, no URLs/domains) that feeds role dashboards; no browsing schema exists |
| Accountability groups and approvals | Implemented supporting workflow | verified partners own multiple groups with hashed/rotatable codes; a verified student has one live membership with category-specific aggregate sharing; authenticated group projections include the partner name plus optional auth-protected avatar route, while the workspace projection exposes consented per-student `block_count_sync` totals over the latest seven `Asia/Jakarta` calendar dates and lets clients distinguish a shared zero through the accompanying sharing preference. `GET /v1/accountability/analytics` returns 14/30-day partner-scoped daily totals plus the hourly "jam rawan" histogram from members who consent to sharing protection activity; partner workspace collections also expose filtered `PaginatedList` contracts for groups, consented members, analytics rows, exits, contacts, and flagged members. Pending normal exits can be cancelled by the requesting student, while unsafe exit remains immediate and support-reviewed; removal, archive, scoped pause/uninstall requests, recent-auth partner decisions, bounded ES256-signed native grants, hashed quick tokens, and set-once P-256 device-key enrollment/proof are wired; Android/Windows native verification remains a client integration boundary. Partnerless removal grants are retired: `uninstall_detected` is issued only through an approved partner request, while `emergency_access` remains the two-admin recovery/removal authority |
...
| Emergency recovery | Implemented operational workflow | protected user requests for an owned device; platform admin reviews and issues within 30 minutes; key is delivered directly via WhatsApp (Fonnte) to the user's registered phone and displayed in admin console; hashed device-bound key is single-use for 24 hours and produces a ten-minute, backend-authored `emergency_access` ES256 grant bound to the enrolled device-key thumbprint; retry preserves grant `jti`, `iat`, and `exp` while valid |
...
| Public application-release catalog | Implemented supporting workflow | `DownloadApp` persists bilingual metadata for the Android APK, Windows MSI, and passive browser-extension ZIPs. A startup seed adds only missing platform records from the checked-in latest stable official GitHub Release snapshot. Public `GET /v1/public/download-apps` returns only published records; verified admins use `GET/PUT /v1/admin/download-apps` with recent auth. Updates require HTTPS, exact official GitHub Release asset prefixes, a direct filename match, a positive byte size, one primary asset, and a lowercase SHA-256 checksum; debug/staging and third-party URLs are rejected. Every save emits `download_app_updated` without user or browsing data. |
...
| WhatsApp delivery | Implemented Fonnte adapter | immediate delivery for phone verification, partner approvals, password reset, data export/deletion, and emergency recovery keys uses configured phone numbers and Fonnte; logs omit tokens, recipients, and message bodies |
...
empty; it creates no demo users or activity. Automatic production deploys run
`migrate-up` plus the guarded `seed-accounts` binary, which installs exactly
the four owner-approved demo accounts with no education, Learning Hub, social,
activity, support, or operational fixtures; the full `demo-seeder` and the
baseline content seeders run only in staging. CI can deploy the private GHCR
