...
binaries. `make migrate-fresh` and guarded `migrate-down` also run
`cmd/reset-storage`, which empties the dynamic storage directories
(media/avatars, encrypted exports, release artifacts) because a dropped schema
