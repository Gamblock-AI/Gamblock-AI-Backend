...
func SeedDataRequests(ctx context.Context, client *ent.Client, now time.Time) error {
	if _, err := client.DataRequest.Create().SetID("DR-1042").SetUserID("usr_gading").SetType("export").SetStatus("completed").SetRequestedAt(now.Add(-24 * time.Hour)).SetCompletedAt(now.Add(-2 * time.Hour)).SetResultPath("./var/exports/DR-1042.zip").Save(ctx); err != nil {
		return err
	}
