...
func SeedSupportCases(ctx context.Context, client *ent.Client) error {
	...
	if _, err := client.SupportCase.Create().SetID("CASE-1084").SetUserID("usr_gading").SetType(supportcase.TypeStuckApproval).SetStatus(supportcase.StatusOpen).SetPriority(supportcase.PriorityHigh).SetSummary("Partner approval issue").Save(ctx); err != nil {
		return err
	}
	return nil
}
