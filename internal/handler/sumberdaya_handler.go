...
func (h *Handler) GetReflections(c *gin.Context) {
	...
	if err != nil {
		h.respondError(c, http.StatusInternalServerError, "fetch_reflections_failed", err.Error())
		return
	}
...
func (h *Handler) CreateReflection(c *gin.Context) {
	...
	if input.Text == "" {
		h.respondError(c, http.StatusBadRequest, "text_required", "Text is required")
		return
	}

	entry, err := h.services.Reflection.CreateReflection(c.Request.Context(), h.currentUserID(c), input.Text, input.Mood)
	if err != nil {
		h.respondError(c, http.StatusBadRequest, "reflection_create_failed", err.Error())
		return
	}
...
func (h *Handler) GetModules(c *gin.Context) {
	...
	if err != nil {
		h.respondError(c, http.StatusInternalServerError, "fetch_modules_failed", err.Error())
		return
	}
...
func (h *Handler) GetModuleDetail(c *gin.Context) {
	...
	if err != nil {
		h.respondError(c, http.StatusNotFound, "module_not_found", err.Error())
		return
	}
...
func (h *Handler) GetSupportCases(c *gin.Context) {
	...
	if err != nil {
		h.respondError(c, http.StatusInternalServerError, "fetch_support_cases_failed", err.Error())
		return
	}
...
func (h *Handler) CreateSupportCase(c *gin.Context) {
	...
	_ = c.ShouldBindJSON(&input)
	if input.Summary == "" {
		h.respondError(c, http.StatusBadRequest, "summary_required", "Summary is required")
		return
	}

	err := h.services.Support.CreateSupportCase(c.Request.Context(), h.currentUserID(c), input.Summary, input.Type, input.Priority)
	if err != nil {
		h.respondError(c, http.StatusBadRequest, "support_case_failed", err.Error())
		return
	}
...
func (h *Handler) GetDataRequests(c *gin.Context) {
	...
	if err != nil {
		h.respondError(c, http.StatusInternalServerError, "fetch_data_requests_failed", err.Error())
		return
	}
...
func (h *Handler) CreateDataRequest(c *gin.Context) {
	...
	_ = c.ShouldBindJSON(&input)
	if input.Type == "" {
		h.respondError(c, http.StatusBadRequest, "type_required", "Type is required")
		return
	}

	err := h.services.Support.CreateDataRequest(c.Request.Context(), h.currentUserID(c), input.Type)
	if err != nil {
		h.respondError(c, http.StatusBadRequest, "data_request_failed", err.Error())
		return
	}
